...
Overall retrospective bias was very low, and close to 0 for all models (@tbl-summary; @fig-retro) indicating a lack of persistant bias in terminal estimates as successive years of the index were removed from the model. Additionally, estimates of the relative exploitation rate ($U/U_{MSY}$) and relative depletion ($D/D_{MSY}$) were inside the credible intervals of the full model run 100% of the time (@tbl-summary). In terms of hindcast cross validation, all models (though `0003-2024cpueFPrior` model showed the best performance) exceeded the performance of the naïve predictor (@tbl-summary; @fig-hcxval) which provides some support that the model has identified and estimated a production function for the stock.
