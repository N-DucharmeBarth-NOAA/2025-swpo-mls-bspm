...
data {
    ...
    // priors
    vector[4] mv_prior_mean;        // mean vector [logK, log_r, log_n]
    ...
    real<lower=0> nu_catch_gamma_shape; // shape param for student-t df
    real<lower=0> nu_catch_gamma_rate; // rate param for student-t df
}
transformed data{
    int Tm1;
    Tm1 = T-1;
    ...
    matrix[4, 4] L_corr = cholesky_decompose(mv_prior_corr);
}
parameters {
    // use mean-zero parameters with SD = 1
    // keeps searched parameter space on a similar scale
    vector[4] raw_mv_params;        // [raw_logK, raw_log_r, raw_log_n]
...
model {
    ...
    // prior for student t degrees of freedom
        nu_catch ~ gamma(nu_catch_gamma_shape, nu_catch_gamma_rate);
    ...
    // observation model
        ...
        for(t in 1:Tm1){
            ...
            target += student_t_lpdf(log(obs_removals[t]) | nu_catch, mu_catch, sigmac[t]);
        }
}
