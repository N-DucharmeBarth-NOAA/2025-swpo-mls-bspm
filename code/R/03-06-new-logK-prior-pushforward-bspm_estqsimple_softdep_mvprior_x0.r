...
# define directories
    ...
    dir_output = file.path(proj_dir,"data","output","pushforward","bspm_estqsimple_softdep_mvprior_x0_newlogK")
    dir.create(dir_output,showWarnings = FALSE, recursive = TRUE)
    dir_plot = file.path(proj_dir,"plots","pushforward","bspm_estqsimple_softdep_mvprior_x0_newlogK")
...
# load inputs
    ...
    # develop initial logK prior assuming max catch is MSY
        calculate_K <- function(MSY, r, n) {
            if (n == 1) {
                # Fox model special case (limit as n approaches 1)
                K <- exp(1) * MSY / r
            } else {
                # General case
                phi <- (1/n)^(1/(n-1))  # inflection point
                factor <- phi * (1 - phi)^(n-1)
                K <- MSY / (r * factor)
            }
            return(K)
        }
    logK_vec = log(apply(cbind(max(catch_effort_annual$total_catch)*rlnorm(nrow(bio_params_dt),0,0.5),bio_params_dt$rmax,bio_params_dt$shape),1,function(x)calculate_K(x[[1]],x[[2]],x[[3]])))    
    # replace non-finite values
    inf_idx = which(!is.finite(logK_vec)|logK_vec>20)
    set.seed(123)
    logK_vec[inf_idx] = sample(logK_vec[-inf_idx],length(inf_idx),replace=TRUE)

    logK_fn = function(par){-sum(dnorm(logK_vec, mean = par[1], sd = par[2], log = TRUE))}
    logK_pars =  nlminb(c(mean(logK_vec), sd(logK_vec)), logK_fn)$par
    ...
    # develop initial catchability prior based on nominal scaled cpue and same logK prior
        ...
        nsim_q = 1e4
        scaled_q = sample(scaled_cpue,nsim_q,replace=TRUE)/sample(exp(logK_vec),nsim_q,replace=TRUE)
    ...
    mv_prior_mean = c(logK_pars[1],as.vector(apply(bio_params_dt[,.(log(rmax),log(shape))],2,mean)),0,qeff_pars_catch[1])
    mv_prior_sd = c(logK_pars[2],as.vector(apply(bio_params_dt[,.(log(rmax),log(shape))],2,sd)),0.025,qeff_pars_catch[2])
...
# calc obs catch and cpue (1000 hooks) reference points
    obs_max_catch = max(stan.data$obs_removals,na.rm=TRUE)
    obs_min_catch = min(stan.data$obs_removals,na.rm=TRUE)
    obs_catch_overall_avg = mean(stan.data$obs_removals,na.rm=TRUE)

    obs_max_cpue_early = max((stan.data$obs_removals/(1e5*stan.data$effort))[c(1:10)],na.rm=TRUE)
    obs_min_cpue_early = min((stan.data$obs_removals/(1e5*stan.data$effort))[c(1:10)],na.rm=TRUE)
    obs_cpue_early_avg = mean((stan.data$obs_removals/(1e5*stan.data$effort))[c(1:10)],na.rm=TRUE)
...
# extract quantities and filter            
        ...
        # apply filtering
            ...
            q_iter_samples = hmc_samples[name%in%c("F")] %>%
                           setnames(.,"value","F") %>%
                           merge(.,data.table(effort=stan.data$effort,row=1:length(stan.data$effort)),by="row") %>%
                           .[,Fnum:=as.numeric(F)] %>%
                           .[,q:=Fnum/effort] %>%
                           .[,.(iter,row,q)]
...
# develop new priors following pushforward
    ...
    # full mv leading...
        full_mv_params = leading_params_dt[type == "cpue", .(logK, r, shape,x0,qeff,rho,sigma_qdev)] %>%
                                .[, .(logK, log_r=log(r),log_shape=log(shape),log_x0=log(x0),log_qeff=log(qeff),atan_rho=atanh(rho), log_sigma_qdev=log(sigma_qdev))] %>%
                                as.matrix(.)

        full_mv_prior_fitted = fit_multivariate_prior(
            param_matrix = full_mv_params,
            param_names = colnames(full_mv_params),
            use_mle = TRUE,
            verbose = TRUE
        )

        pp_full_mv_prior_mean = full_mv_prior_fitted$mean
        pp_full_mv_prior_sd = full_mv_prior_fitted$sd
        pp_full_mv_prior_cor = full_mv_prior_fitted$cor
        # reset cor to 0 for low-corr pairs
        pp_full_mv_prior_cor[which(abs(pp_full_mv_prior_cor)<0.1)] = 0
...
# generate samples from new prior and compare
    ...
    # full mv
        new_samples_full_mv_cov = diag(pp_full_mv_prior_sd) %*% pp_full_mv_prior_cor %*% diag(pp_full_mv_prior_sd)
        new_samples_full_mv = MASS::mvrnorm(plot_samples, mu = pp_full_mv_prior_mean, Sigma = new_samples_full_mv_cov)

        new_samples_dt = as.data.table(new_samples_full_mv) %>%
        .[,type:="updated prior"] %>%
...
# save new stan.data object
    ...
    updated_stan_data$full_mv_prior_mean = unname(pp_full_mv_prior_mean)
    updated_stan_data$full_mv_prior_sd = unname(pp_full_mv_prior_sd)
    updated_stan_data$full_mv_prior_corr = unname(pp_full_mv_prior_cor)
