...
generate_mj <- function(model_dirs, params = NULL) {
  ...
  if(uniqueN(plot_dt$run_label)>1){  
    p = p + geom_path(aes(x = D, y = F_Fmsy, linetype = type, color = run_label), linewidth = 1.25)
  } else {
    p = p + geom_path(aes(x = D, y = F_Fmsy, linetype = type),color = "gray60", linewidth = 1)
    p = p + geom_point(aes(x = D, y = F_Fmsy, fill = year),color = "black", shape=21,size=2)
  }
  ...
  if (!is.null(contour_dt)) {
    ...
    if(uniqueN(plot_dt$run_label)>1){  
      ...
      p <- p + geom_polygon(data = contour_dt, 
                          aes(x = D, y = F_Fmsy, group = plot_id), fill="gray60", 
  ...
  if(uniqueN(plot_dt$run_label)>1){  
    ...
    p <- p +
    geom_point(data = plot_dt[year == min(plot_dt$year)], aes(x = D, y = F_Fmsy, color = run_label), 
              shape = 21, fill = "white", size = 3) +
    geom_point(data = plot_dt[year == max(plot_dt$year)], aes(x = D, y = F_Fmsy), 
